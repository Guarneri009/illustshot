...
bool EdgeDetectionSample::processFrame(const cv::Mat& inputFrame, cv::Mat& outputFrame)
{
  ...
  if (m_showOnlyEdges)
  {
    ...
    cv::cvtColor(grayImage, outputFrame, CV_GRAY2BGRA);
  }
  else 
  {
...
//! Processes a frame and returns output image 
bool EdgeDetectionSample::processFrame2(const cv::Mat& inputFrame, cv::Mat& outputFrame)
{
	...
	//画像処理例（ガウシアンフィルタ）
	GaussianBlur(grayImage, temp1, ksize, 0.3);
	//2値化
	cv::threshold(temp1, dst, 100, 255, cv::THRESH_BINARY);
	cv::threshold(grayImage, edges, 0, 255, cv::THRESH_BINARY|cv::THRESH_OTSU);

	// 入力画像，出力画像，maxVal，閾値決定手法，閾値処理手法，blockSize，C
	//cv::adaptiveThreshold(src2, dst, val, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 5, 8);

	//カラーをセット
	cv::cvtColor(dst, outputFrame, CV_GRAY2BGRA);
